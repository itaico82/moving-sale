...
(function () {
  ...
  var ICON = {
    ...
    email:
      '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="4" width="20" height="16" rx="2"></rect><path d="m22 7-10 5L2 7"></path></svg>',
    share:
      '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="18" cy="5" r="3"></circle><circle cx="6" cy="12" r="3"></circle><circle cx="18" cy="19" r="3"></circle><path d="M8.59 13.51l6.83 3.98M15.41 6.51l-6.82 3.98"></path></svg>',
  };
  ...
  function cardHTML(item, t) {
    ...
    var media =
      ...
      '<div class="ms-share-wrap">' +
        '<button class="ms-share" data-action="toggle-share" title="' + esc(t.shareTitle) + '" aria-label="' + esc(t.shareTitle) + '">' + ICON.share + "</button>" +
        '<div class="ms-share-menu" data-share-menu>' +
          '<a class="ms-share-opt" data-action="share-go" href="' + esc(waShare) + '" target="_blank" rel="noopener">' + ICON.whatsapp + "<span>" + esc(t.shareWa) + "</span></a>" +
          '<a class="ms-share-opt" data-action="share-go" href="' + esc(mailShare) + '">' + ICON.email + "<span>" + esc(t.shareMail) + "</span></a>" +
        "</div>" +
      "</div>" +
      "</div>";
  ...
  function onClick(e) {
    ...
    switch (action) {
      ...
      case "share-go":
        // let the anchor open WhatsApp/email normally; just close the menu
        closeShareMenus();
        break;
    }
  }
  ...
  /* ---------- boot ---------- */
  function handleHash() {
    var m = (location.hash || "").match(/item=([^&]+)/);
    if (m) { try { openItemModal(decodeURIComponent(m[1])); } catch (e) {} }
  }
